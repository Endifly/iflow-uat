...
class _ConcaveDecorationPainter extends BoxPainter {
  ...
  List<Color> getColor() {
    if (colors == null) return [Colors.black87, Colors.white];
  ...
  void paint(
    ...
    final rect = offset & (configuration.size as Size);
    final shapePath = shape.getOuterPath(rect);

    final delta = 8 / rect.longestSide;
    ...
    for (final alignment in [Alignment.topLeft, Alignment.bottomRight]) {
      ...
      paint
        ..shader = ui.Gradient.linear(
            shaderRect.topLeft, shaderRect.bottomRight, getColor(), stops);
